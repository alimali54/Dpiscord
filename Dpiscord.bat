...
for /f "tokens=*" %%g in ('curl -s -H "accept: application/dns-json" "https://1.1.1.1/dns-query?name=updates.discord.com&type=A"') do (
    ...
    for %%h in (!JSON_OUT!) do (
        ...
        if !errorlevel! equ 0 (
            ...
            set "SAFE_IP=!SAFE_IP:}=!"
            set "SAFE_IP=!SAFE_IP:]=!"
            :: "data:" metnini bul ve sil (Vurucu darbe)
...
:COMPARE
:: Karþýlaþtýrma (Ýlk 5 hane: 162.1 gibi)
...
if "%LOCAL_COMP%" neq "%SAFE_COMP%" (
    ...
    echo/
    echo !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    echo HATA: DNS Zehirlenmesi Saptandý! ^(ISS Müdahalesi Var^)
    echo Yerel IP: %LOCAL_IP% ^| Gerçek IP: %SAFE_IP%
    echo/
    echo Servis saðlayýcýnýz DNS isteklerinizi manipüle ediyor.
